...
## After `hermes update` — cua-driver upgrade

`hermes update` also upgrades cua-driver automatically. When the binary changes,
macOS may surface a one-time re-grant prompt for Accessibility and/or Screen
Recording because the new binary's cdhash doesn't match the old one's.

TCC grants are keyed on bundle ID (`com.trycua.driver`), which is preserved across
upgrades, so they usually transfer silently. But if computer use stops working
immediately after a Hermes update:

```bash
cua-driver --version           # confirm the upgrade landed
cua-driver permissions status  # check grant state
# If not granted:
cua-driver permissions grant   # launches CuaDriver so the dialog says "Cua Driver"
```

The `permissions grant` flow is the right way — it launches the app under its own
identity so the TCC dialog names the driver, not your terminal. After approving,
grants persist until the next binary change.

## Setup / permission repair workflow

When Hermes doctor says `computer_use (system dependency not met)`, check and repair in this order:

```bash
hermes computer-use status
hermes computer-use install
cua-driver --version
hermes doctor | grep -i computer_use
```

If `cua-driver` is installed but permissions are unclear, launch the driver daemon under its own macOS TCC identity before checking permissions:
...
To help the user find the two macOS panes directly:
...
On newer macOS, Apple labels Screen Recording as **Screen & System Audio Recording**. The user must enable **Cua Driver** in both Accessibility and Screen & System Audio Recording. A standalone `cua-driver check_permissions` can misleadingly report the terminal's grants; prefer `permissions status` with the daemon running.

## Failure modes

- **"cua-driver not installed"** — Run `hermes computer-use install` or use `hermes tools` and enable Computer
  Use; the setup will install cua-driver via its upstream script. Requires
  macOS + Accessibility + Screen Recording permissions.
- **Vision analysis returns "No LLM provider configured"** — The `auxiliary.vision` model must be a vision-capable model. `deepseek/deepseek-v4-flash` does NOT support vision. Switch to `google/gemini-2.5-flash`, `anthropic/claude-sonnet-4-6`, or `openai/gpt-4o` via:
  ```
  hermes config set auxiliary.vision.model google/gemini-2.5-flash
  ```
  The config change takes effect on the next vision call — no restart needed.
- **Chrome AX tree not accessible** — The CUA driver sometimes can't find Chrome's window, especially when Chrome is on a different Space or just launched. Workaround: use `osascript -e 'tell application "Google Chrome" to activate'` to bring Chrome forward, then capture without an `app` filter: `computer_use(action="capture", mode="som")`. SOM mode with the full desktop works more reliably than `app="Google Chrome"`.
- **`raise_window=true` when user asks you to drive their computer** — The background-mode rule ("never raise_window") is the default, but when the user explicitly says "I want you to use my computer" or "do this for me," raising windows is necessary for the driver to find them. Steve's explicit instruction overrides the default.
